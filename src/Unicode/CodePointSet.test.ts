...
test("constructor: with no args - allows access to full set of code points and stores under correct key", () => {
  const cps = new CodePointSet();
  expect(cps.select(/./sv).length).toEqual(1114111);
  expect(cache["all->/./gsv"].length).toEqual(1114111);
  expect(cps.select(/./s).length).toEqual(65536);
  expect(cache["all->/./gs"].length).toEqual(65536);
});
